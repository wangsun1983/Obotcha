...
_LibraryFileInputStream::_LibraryFileInputStream(LibraryFile f) { mFile = f; }
...
int _LibraryFileInputStream::read() {
    Trigger(MethodNotSupportException,
            "LibraryFileInputStream not support read");
}
...
bool _LibraryFileInputStream::open() {
    ...
    mHandle =
        dlopen(mFile->getAbsolutePath()->toChars(), RTLD_LAZY | RTLD_GLOBAL);
