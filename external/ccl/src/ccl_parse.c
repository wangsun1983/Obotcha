...
int ccl_parse_content(struct ccl_t *data, const char *content)
{
	...
	for (p = (char *)content; p < content + count; /* ++p */)
	{
		...
		case CCL_PARSE_QUOTED:
			...
			{
				fprintf(stderr, PACKAGE ": Unterminated string content\n");
				state = CCL_HANDLE_NEWLINE;
			}
			else
			{
		...
		case CCL_HANDLE_SEP:
			...
			{
				pair = 0;
				fprintf(stderr, PACKAGE ": Missing key\n");
			}
			else if (ccl_get(data, token) != 0)
			{
				pair = 0;
				fprintf(stderr, PACKAGE ": Ignoring duplicate key '%s' \n",
						token);
			}
			else
			{
