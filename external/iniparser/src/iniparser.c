...
// wangsl
dictionary *iniparser_load_content(const char *c)
{
    ...
    while(content <= end && !isComplete)
    {
        //find next line
        for(next = content;next <= end;next++) {
            if(*next == '\n' || *next == '\r') {
                *next = 0;
                break;
            }
        ...
        switch (iniparser_line(content, section, key, val))
        {
        case LINE_EMPTY:
        case LINE_COMMENT:
            break;
