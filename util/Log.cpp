...
void SetLogFile(String infoPrefix,String warningPrefix,String errorPrefix,String fatalPrefix) {
    if(infoPrefix != nullptr) {
        google::SetLogDestination(google::GLOG_INFO, infoPrefix->toChars());
    }

    if(warningPrefix != nullptr) {
        google::SetLogDestination(google::GLOG_WARNING, warningPrefix->toChars());
    }
