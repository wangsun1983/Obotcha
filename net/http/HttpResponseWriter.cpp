...
void _HttpResponseWriter::disableResponse() { mResponsible = false; }

bool _HttpResponseWriter::isResponsible() { return mResponsible; }
...
int _HttpResponseWriter::write(HttpResponse response, bool flush) {
    ...
    ArrayList<KeyValuePair<String, String>> encodedValues =
        response->getEntity()->getEncodedKeyValues();
    ...
    AUTO_FLUSH(writer->writeString(
        response->getHeader()->toString(st(HttpProtocol)::HttpResponse)));
    ...
    if (file != nullptr && file->exists()) {
        if (file->exists()) {
            ...
            if (response->getType() == st(HttpResponse)::CHUNCKED) {
                while (filesize != 0) {
                    int readlength = mSendBuff->size() - writer->getIndex() -
                                     32 /*reserve data */;
                    ...
                    filesize -= readlength;
                    AUTO_FLUSH(writer->writeString(
                        createInteger(readlength)->toHexString()));
                    AUTO_FLUSH(writer->writeString(st(HttpText)::CRLF));
