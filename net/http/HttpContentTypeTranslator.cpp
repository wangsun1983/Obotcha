...
void _HttpContentTypeTranslator::addNode(FileTypeSearchNode *root,
...
int _HttpContentTypeTranslator::searchNode(FileTypeSearchNode *root,
                                           const char *content, int size) {
    int index = content[0] - 0x21;
    if (root->next[index] == nullptr) {
        return -1;
...
#define ADD_NODE(X, Y)                                                         \
    addNode(mSuffixRootNode, st(HttpContentType)::X->toChars(),                \
            st(HttpContentType)::X->size(), st(HttpContentType)::Y)
...
#define ADD_NODE(X, Y)                                                         \
    addNode(mContentTypeNode, st(HttpContentType)::X->toChars(),               \
            st(HttpContentType)::X->size(), st(HttpContentType)::Y)
