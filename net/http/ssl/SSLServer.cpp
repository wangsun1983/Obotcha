...
_SSLThread::_SSLThread(String ip, int port, SocketListener l, String c,
                       String k) {
    String mIp = ip;
    ...
    const int EPOLL_SIZE = st(Enviroment)::getInstance()->getInt(
        st(Enviroment)::gTcpServerEpollSize, 1024);
    if (l == nullptr) {
    ...
    while (1) {
        ...
        mRcvBuffSize = st(Enviroment)::getInstance()->getInt(
            st(Enviroment)::gTcpServerRcvBuffSize, 1024);

        mClientNums = st(Enviroment)::getInstance()->getInt(
            st(Enviroment)::gTcpServerClientNums, 1024);
        ...
        if (setsockopt(mSocket, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(int)) <
            0) {
            break;
        }

        if (bind(mSocket, (struct sockaddr *)&mSockAddr,
...
void _SSLThread::run() {
    ...
    while (1) {
        ...
        for (int i = 0; i < epoll_events_count; ++i) {
            ...
            if (((event & EPOLLIN) != 0) && ((event & EPOLLRDHUP) != 0)) {
                ...
                mListener->onSocketMessage(st(SocketListener)::Disconnect, s,
                                           nullptr);
            ...
            if (sockfd == mSocket) {
                ...
                if (ssl->bindSocket(clientfd) == 0) {
                    ...
                    mListener->onSocketMessage(st(SocketListener)::Connect, s,
                                               nullptr);
                ...
                if (mListener != nullptr) {
                    ...
                    mListener->onSocketMessage(st(SocketListener)::Message, s,
                                               buff);
                }
            }
        }
...
//----------------- HttpsServer -----------------
_SSLServer::_SSLServer(int port, SocketListener l, String c, String k)
    : _SSLServer(nullptr, port, l, c, k) {}

_SSLServer::_SSLServer(String ip, int port, SocketListener l, String c,
                       String k)
    : _SSLServer(ip, port,
                 st(Enviroment)::getInstance()->getInt(
                     st(Enviroment)::gTcpServerRcvBuffSize, 1024 * 32),
                 st(Enviroment)::getInstance()->getInt(
                     st(Enviroment)::gTcpServerClientNums, 1024 * 32),
                 l, c, k) {}

_SSLServer::_SSLServer(String ip, int port, int rcvBuffsize, int connectionsNum,
                       SocketListener l, String c, String k) {
...
void _SSLServer::start() { mSSLThread->start(); }
