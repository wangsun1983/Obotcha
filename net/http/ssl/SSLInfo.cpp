...
_SSLInfo::_SSLInfo(String certificate, String key) {
    ...
    if (certificate == nullptr) {
        mCertificate = st(Enviroment)::getInstance()->get(
            st(Enviroment)::gHttpSslCertificatePath);
    }

    if (mKey == nullptr) {
        mKey =
            st(Enviroment)::getInstance()->get(st(Enviroment)::gHttpSslKeyPath);
    }
...
int _SSLInfo::getSocket() { return mSock; }
...
SSL *_SSLInfo::getSSL() { return mSSL; }
